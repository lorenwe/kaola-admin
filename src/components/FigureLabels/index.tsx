...
import { isSuccess, randomTagColor } from '@/utils/tools'
...
const FigureLabels: FC<IProps> = ({ value, onChange, canCallback }) => {
	console.log("FigureLabels", value, canCallback)
	// 获取全局状态
	...
	const { run: runUpdateUser } = useRequest(updateUser, {
		manual: true,
		onSuccess: async ({ code, message }, params) => {
			if (isSuccess(code)) {
				myMessage.success(message as string)
				// 更新全局状态
				if (params[0]?.tags && initialState?.current_user?.user_id && initialState?.current_user?.user_id === parseInt(params[0]?.user_id)) {
					// console.log(initialState?.current_user)
					let new_tags = params[0]?.tags
					let new_current_user = { ...initialState?.current_user, tags: new_tags }
					setInitialState((s) => ({...s, current_user: new_current_user}));
				}
			}
		},
	},
	...
	const handleClose = (removedTag: string) => {
		...
		// 判断是否需要更新用户信息
		if (canCallback) {
			const user_id =  String(initialState?.current_user?.user_id as number)
			if (user_id) {
		...
		<>
			...
			{/* 控制新增标签和 input 的显示 */}
			{
				...
				/> : (
					tags.length <= 15 ?
						<Tag
							onClick={() => { setInputVisibleTrue(); setEditInputIndex(-1) }}
							style={{ marginTop: 5 }}
						>
