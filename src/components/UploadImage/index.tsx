...
import { formatPerfix, getLocalStorageItem, isHttpLink, isImageFile } from '@/utils/tools'
...
const UploadImage: FC<UploadImageProps> = ({
  ...
  const onChangeUpload: UploadProps['onChange'] = ({ file, fileList }: UploadChangeParam<UploadFile>) => {
    ...
    if (file.status === 'removed') {
      ...
      if (file.status === 'done') {
        ...
        // 获取当前上传图片路径
        const path: string = get(file, 'response.data.url')
        console.log('上传完成', path)
  ...
  const beforeUpload = (file: RcFile) => {
    ...
    const isFileType = isImageFile(file.type, [accept])
    // console.log('beforeUpload',maxSize, file.size / 1024 / 1024)
    // 图片大小限制
